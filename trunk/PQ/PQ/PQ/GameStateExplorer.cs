...
namespace PQ
{
    public class GameStateExplorer : GameState
    {
        ...
        {
            _globalMap = (GlobalMap) this.Game.MapManager.CreateObject(0);
            this.ManageObjects(_globalMap);
            this._gameObjects.Add(_globalMap);
            _listBuilding = new List<GameBuilding>();
            GameBuilding tmp = (GameBuilding)this.Game.GameBuildingManager.CreateObject(0);
        ...
        {
            Rectangle gameBound = Game.GraphicsDevice.PresentationParameters.Bounds;
